...
func New(ctx context.Context, options *Options) (_ *API, err error) {
	...
	if err != nil {
		...
		if xerrors.As(err, &derr) {
			return nil, xerrors.Errorf("database encrypted with unknown key, either add the key or see https://docs.coder.buildworkforce.ai/admin/encryption#disabling-encryption: %w", derr)
		}
